...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Sequential(
            ...
            get_activation(act_type, inplace=True)
        )
...
class FeatureFusor(nn.Module):
    ...
    def forward(self, inputs):
        p3, p4, p5, p6 = inputs
        p5 = self.p6_upsample(self.p6_deform_conv_2(p6)) + p5
...
class YOLOXMOTHead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        reid_features = self.feature_fuse(xin)
    ...
    def get_losses(
        ...
        # print("!!!!!!!!:", labels.shape)  # [batch, n_anchors_all, 6]
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            # Get Track IDs
            gt_track_ids_per_image = labels[batch_idx, :num_gt, 5]
            ...
            reid_head = F.normalize(t_reid_feat_vects)
            cls_id_pred = self.fc.forward(reid_head).contiguous()

            # --- Get reID Loss for This Image for This Class
            reid_loss = self.reid_loss(cls_id_pred, gt_track_ids_per_image.long())
        ...
        # calculate all loss
        reid_loss /= num_fg
        loss = math.exp(-self.s_det)*loss+math.exp(-self.s_id)*reid_loss+(self.s_det+self.s_id)
